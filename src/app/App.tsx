...
function App() {
  ...
  return (
    <>
      ...
      />
      <select onChange={handleFilter} className="border rounded-md dark:border-gray-300 border-gray-900 bg-gray-900 p-2 max-w-72 w-full mt-2">
      ...
      <div className="mt-4 mx-10 max-w-200 w-full">
        {links.map((link) => (
          <div key={link.id} className="bg-gray-900 p-2 mt-1 rounded-md flex justify-between flex-wrap">
            <div className="flex items-center gap-2">
              <img src={link.icon} className="size-6" alt="icon del sitio" />
              <a href={link.url} target="_blank" rel="noreferrer" className="text-blue-500 text-lg">{link.text}</a>
            </div>
            <p className="text-gray-300 flex items-center">{link.categoria}</p>
          </div>
        ))}
      </div>
    </>
